...
export async function redigerLectureSeo(
  ...
  const lignes: string[] = [
    ...
    "",
    "Chantiers SEO déclarés :",
    ...lecture.chantiers.map((c: any) => `- ${c.date} « ${c.libelle} » cible ${c.cible ?? "n/a"}${c.hypothese ? `, hypothèse : ${c.hypothese}` : ""}${c.verdict ? `, verdict posé : ${c.verdict}` : ", verdict : aucun"}${c.mesure_prevue_le ? `, mesure prévue le ${c.mesure_prevue_le}` : ""}`),
    "",
    "Highlights calculés :",
    ...highlights.map((h) => `- ${h}`),
  ];
