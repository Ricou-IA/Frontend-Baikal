...
**Proposition** :
1. Remplacer le point « **Vue d'ensemble par site** » (lignes 278-281) par : « **Mesures par chapitre** : contrat `docs/contrats/mesures-v1.sql` — le site publie une vue `baikal_mesures` (une ligne par jour, clé et fenêtre ; `agregation` somme|dernier ; `chapitre` clients|finances|comptes_pro ; `rendu` tuiles|entonnoir), Baikal la lit par l'action `mesures` de l'EF `admin-site-stats` et affiche les tuiles en tête du chapitre (`src/components/console/BandeauMesures.jsx`). Pas d'écran des statistiques. Tout ce qui décide d'un nombre affiché est dans `admin-site-stats/mesures.ts`, pur et testé. Bornes de fenêtre calculées en TypeScript, jamais en SQL (postgres.js envoie ses paramètres sans type : `$1::date - $2` a été résolu en soustraction de deux dates). L'action `overview` et `stats-sites.ts` survivent pour pack-vendeur, voirie et majordhome jusqu'à leur bascule, ce qui garde `VueSite` dans `Admin.jsx`. Brancher un site = publier sa vue et passer la recette du contrat, aucun code Baikal. Branché : monsieurdpe. Spec : `docs/superpowers/specs/2026-09-23-contrat-mesures-design.md`. »
2. Ajouter après le point Clients : « **Comptes pro** : page `/comptes-pro` (`src/pages/ComptesPro.jsx`) + EF `admin-comptes-pro` — les entreprises qui achètent au site (Clients liste l'acte, Comptes pro le compte). Contrat `docs/contrats/comptes-pro-v1.sql` : vue `baikal_comptes_pro`, noyau obligatoire, blocs optionnels déclarés par leur colonne pivot (`credits_stock`, `ca_ttc`, `abo_statut`). Une liste ne porte jamais d'agrégat : les tuiles viennent de `baikal_mesures`. Branché : monsieurdpe. »
3. Ligne 307, liste des modules : ajouter `comptes_pro` après `clients`.
4. Ligne 256, colonnes propres au site : ajouter « `fuseau` (calendrier des mesures, défaut Europe/Paris ; Baikal y pose ses bornes de fenêtre) ».
